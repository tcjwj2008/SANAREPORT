...
{
    public partial class ValueMenuForm : SMes.Controls.ExtendForm.BaseForm
    {
        string _userid = string.Empty;
        string _typeid = string.Empty;
        public ValueMenuForm(string typeid, string userid)
        {
            _typeid = typeid;
            _userid = userid;
            InitializeComponent();
        ...
        private void navigatorEx1_OnDelete(object sender, SysButtonClickedEventArgs e)
        {
            if (this.dataGridViewEx1.SelectedRows != null && this.dataGridViewEx1.SelectedRows.Count > 0)
            {
                ...
                //{
                //    //检测是否已经保存，没保存要先保存
                //    string dataid = SMes.Core.Utility.StrUtil.ValueToString(this.dataGridViewEx1.SelectedRows[i].Cells[this.ColValueID.Name].Value);
        ...
        {
            this.navigatorEx1.QuerySql = Sql.LookUpSql.SearchValueData(_typeid, _userid);
        }
