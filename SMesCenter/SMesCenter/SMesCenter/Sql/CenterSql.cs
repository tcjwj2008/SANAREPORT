...
namespace SMesCenter.Sql
{
    class CenterSql
    {
        ...
        public static string organizationsql()
        {
        ...
        public static string InitFunctionName()
        {
            string sql = @" select id as 功能编号,functionname 功能名称,functionpath 功能路径,creater 创建者,useNum as 使用次数,
lastuser as 最后使用者,lastusedate  最后使用时间,memo 备注 from functionname 

            where   1 = 1 order by useNum desc";
            return sql;
        }
        public static string InitFunctionName1()
        {
         string sql = @" select id as 功能编号,functionname 功能名称,functionpath 功能路径,creater 创建者,useNum as 使用次数,
lastuser as 最后使用者,lastusedate  最后使用时间,memo 备注 from functionname 

            where orgid=1 and 1 = 1 order by useNum desc";
            return sql;
        }
        public static string InitFunctionName2()
        {
            string sql = @" select id as 功能编号,functionname 功能名称,functionpath 功能路径,creater 创建者,useNum as 使用次数,
lastuser as 最后使用者,lastusedate  最后使用时间,memo 备注 from functionname 

            where orgid=2 and 1 = 1 order by useNum desc";
            return sql;
        }
        public static string InitFunctionName3()
        {
            string sql = @" select id as 功能编号,functionname 功能名称,functionpath 功能路径,creater 创建者,useNum as 使用次数,
lastuser as 最后使用者,lastusedate  最后使用时间,memo 备注 from functionname 

            where orgid=3 and 1 = 1 order by useNum desc";
            return sql;
        }
        public static string InitFunctionName4()
        {
            string sql = @" select id as 功能编号,functionname 功能名称,functionpath 功能路径,creater 创建者,useNum as 使用次数,
lastuser as 最后使用者,lastusedate  最后使用时间,memo 备注 from functionname 

            where orgid=4 and 1 = 1 order by useNum desc";
            return sql;
        }
        public static string InitFunctionName5()
        {
            string sql = @" select id as 功能编号,functionname 功能名称,functionpath 功能路径,creater 创建者,useNum as 使用次数,
lastuser as 最后使用者,lastusedate  最后使用时间,memo 备注 from functionname 

            where orgid=5 and 1 = 1 order by useNum desc";
            return sql;
        }

        public static string InitFunctionName_Orgid(string orgid)
        {
            string sql = string.Format(@" select id as 功能编号,functionname 功能名称,functionpath 功能路径,creater 创建者,useNum as 使用次数,
lastuser as 最后使用者,lastusedate  最后使用时间,memo 备注 from functionname 

            where orgid='{0}' and 1 = 1  order by useNum desc",orgid);
            return sql;
        }


        public static string InitFunctionName_Orgid_Username(string orgid,string username)
        {
					string sql = string.Format(@" select functionid as 功能编号,functionname 功能名称,functionpath 功能路径,creater 创建者,useNum as 使用次数,
lastuser as 最后使用者,lastusedate  最后使用时间,memo 备注 from smes_functionname 

            where orgid='{0}' and 1 = 1  and functioncode in (SELECT C.functionname FROM smes_function_user C ,smes_users B WHERE CONVERT(VARCHAR(50),C.userid)=CONVERT(VARCHAR(50),B.USER_NAME) AND B.USER_NAME='{1}') order by useNum desc", orgid, username);
            return sql;
        }

				public static string InitFunctionName_Orgid_UsernameOLD(string orgid, string username)
				{
					string sql = string.Format(@" select functionid as 功能编号,functionname 功能名称,functionpath 功能路径,creater 创建者,useNum as 使用次数,
lastuser as 最后使用者,lastusedate  最后使用时间,memo 备注 from smes_functionname 

            where orgid='{0}' and 1 = 1  and functionname in (SELECT C.functionname FROM smes_function_user C ,smes_users B WHERE CONVERT(VARCHAR(50),C.userid)=CONVERT(VARCHAR(50),B.USER_NAME) AND B.USER_NAME='{1}') order by useNum desc", orgid, username);
					return sql;
				}


        public static string InitConfig_New()
        {
            string sql = @"select config_name,config_value from config_New where 1 = 1";
            return sql;
        }
        ...
        /////////开始数据库的访问sql
        public static string GetUserInfoSqlORACLE(string userName)
        {
            string sql = "select t.user_id,t.user_name,t.user_password,t.true_name,t.start_date,t.end_date from SMES_USERS t WHERE t.user_name = '" + userName + "'";
            return sql;
        }
        ...
        {
            string sql = string.Format(@"SELECT a.assemble + '#' + MAX(a.version_number)
  FROM (SELECT sa.assemble, sa.version_number
  FROM smes_assemble sa
  WHERE  sa.platform = 'PC'
    AND sa.function_id IN (SELECT DISTINCT mb.functionname
                            FROM smes_function_user mb
                           WHERE mb.userid='{0}'))a
group by a.assemble,a.version_number ", userId);

            return sql;
        }
				public static string GetUserRefAssemblesORACLE(string userId)
				{
					string sql = string.Format(@"SELECT a.assemble + '#' + MAX(a.version_number)
  FROM (SELECT sa.assemble, sa.version_number
  FROM smes_assemble sa
...
group by a.assemble,a.version_number ", userId);
        ...
        public static string GetUserRefAssemblesORALCE(string userId)
        {
...
INSERT INTO smes_assembly_entry_log
                          (entry_log_id, function_id, user_id, organization_id, entry_date, creation_date, created_by)
                        VALUES
                          (@AA, '" + functionId + "', '" + userId + "', '" + orgId + "', getdate(), getdate(),  '" + userId + "')";

					return sql;
				}
        public static string GetAssemblyEntryInsertSqlOracle(string userId,string orgId,string functionId)
        {
        ...
        public static string GetAssemblyEntryInsertSql_New(string userId, string orgId, string functionId,string functionname)
        {
            string sql = @"INSERT INTO smes_assembly_entry_log
                          (entry_log_id, function_id, user_id, organization_id, entry_date, creation_date, created_by)
                        VALUES
        ...
        public static string UpdateFunctionUseNum(string userId,  string functionId, string functionname)
        {
            string sql = string.Format(@"update smes_functionname  set lastuser='{0}',lastusedate='{3}', functionname='{2}' ,useNum=useNum+1 where functionid={1} and useNum is not null",
                        userId,functionId,functionname,DateTime.Now.ToString()
                      );

            return sql;
        }
				public static string UpdateFunctionUseNumold(string userId, string orgId, string functionId, string functionname)
				{
					string sql = string.Format(@"update smes_functionname  set lastuser='{0}',lastusedate='{4}', functionname='{3}' ,orgid='{1}',useNum=useNum+1 where function_id={2} and useNum is not null",
											userId, orgId, functionId, functionname, DateTime.Now.ToString()
										);
