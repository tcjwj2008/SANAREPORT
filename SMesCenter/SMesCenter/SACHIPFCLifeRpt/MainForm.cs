...
namespace SACHIPFCLifeRpt
{
    public partial class MainForm : SMes.Controls.ExtendForm.BaseForm
    {
        ...
        private void dgvLife_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                string curId = SMes.Core.Utility.StrUtil.ValueToString(this.dgvLife.Rows[e.RowIndex].Cells[this.ColHPotID.Name].Value);
                if (curId != _potID)
                {
                    _potID = curId;
        ...
        private void dgvLife_RowPrePaint(object sender, DataGridViewRowPrePaintEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                try
                {
                    DataGridViewRow dg = dgvLife.Rows[e.RowIndex];
                    if (e.RowIndex % 2 == 0)
                    {
        ...
        private void navigatorEx1_OnQuerySuccess(object sender, SMes.Controls.AppObject.SysButtonClickedEventArgs e)
        {
            ...
            {
                _potID = SMes.Core.Utility.StrUtil.ValueToString(this.dgvLife.Rows[0].Cells[this.ColHPotID.Name].Value);
