...
{
    public partial class BatchForm : SMes.Controls.ExtendForm.BaseForm
    {
        ...
        public BatchForm(string userId)
        {
            _userId = userId;
           // _orgId = orgId;
            InitializeComponent();
            ...
            try
            {
                ...
                for (int y = 0; y < this.dgvUsers.Rows.Count; y++)
                {
                    ...
                    if (userCheck.CompareTo("TRUE") == 0)
                    {
                        ...
                        DataTable dtUser = SMes.Core.Service.DataBaseAccess.GetQueryData(Sql.UserDefMenuRefSql.GetUserIdSql(curUserName));
                        ...
                        for (int i = 0; i < this.dgvMenuFunc.Rows.Count; i++)
                        {
                            ...
                            string menuId = SMes.Core.Utility.StrUtil.ValueToString(this.dgvMenuFunc.Rows[i].Cells[this.ColMenuId.Name].Value);

                            if (checkFlag.CompareTo("TRUE") == 0)
                            {
                                /////查询是否存在，用户ID，MenuId
                                DataTable dt = SMes.Core.Service.DataBaseAccess.GetQueryData(Sql.UserDefMenuRefSql.GetUserPerMenuCountSql(curUserId, menuId));
                                int count = SMes.Core.Utility.StrUtil.ValueToInt(dt.Rows[0][0]);

                                if (count == 0)
                                {
                                    ///////进行插入
                                    string sql = Sql.UserDefMenuRefSql.GetUserPerMenuInsertSql(_userId,  menuId);
        ...
        {
            this.navigatorEx1.QuerySql = Sql.UserDefMenuRefSql.GetUserMenuSql(_userId);
        }
