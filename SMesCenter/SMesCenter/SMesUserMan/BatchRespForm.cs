...
{
    public partial class BatchRespForm : SMes.Controls.ExtendForm.BaseForm
    {
        string[] columnDataArray = null;
        string _userId = string.Empty;
        bool allCheck = false;

        public BatchRespForm(string userId)
        {
            _userId = userId;
            InitializeComponent();
        }
                ...
                for (int y = 0; y < this.dgvUsers.Rows.Count; y++)
                {
                    ...
                    if (userCheck.CompareTo("TRUE") == 0)
                    {
                        ...
                        for (int i = 0; i < this.dgvMenuFunc.Rows.Count; i++)
                        {
                            ...
                            string respId = SMes.Core.Utility.StrUtil.ValueToString(this.dgvMenuFunc.Rows[i].Cells[this.ColRespId.Name].Value);

                            if (checkFlag.CompareTo("TRUE") == 0)
                            {
                                /////查询是否存在，用户ID，respId
                                DataTable dt = SMes.Core.Service.DataBaseAccess.GetQueryData(Sql.UserManSql.GetUserPerRespCountSql(curUserId, respId));
                                int count = SMes.Core.Utility.StrUtil.ValueToInt(dt.Rows[0][0]);

                                if (count == 0)
                                {
                                    ///////进行插入
                                    string sql = Sql.UserManSql.InsertBatchUserResp(_userId, curUserId, respId);
        ...
        {
            this.navigatorEx1.QuerySql = Sql.UserManSql.GetRespBatchListSql();
        }
