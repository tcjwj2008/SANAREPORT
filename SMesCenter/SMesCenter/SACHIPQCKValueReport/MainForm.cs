...
{
    public partial class QCKValueReport : SMes.Controls.ExtendForm.BaseForm
    {
        string UserID;
        string _userId;

        public QCKValueReport()
        {
        ...
        private void LoadDefault()
        {
            try
            {
                ...
                //if (string.IsNullOrEmpty(_usderid) && !Debugger.IsAttached)
                //{
                //    MessageBox.Show("无法直接开启程序，请从Cimcenter入口进入", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                //    this.Close();
                //}
                //SMes.Core.Service.DataBaseAccess.SetDataBaseAccType(SMes.Core.Utility.DataBaseType.CHIPDM, _usderid);
        ...
        private void QCKValueReport_Load(object sender, EventArgs e)
        {
            try
            {
                LoadDefault();
